<?php
class OpcrFinalRatingCollection extends Helper {
    ...
    public function get_opcr(){
        ...
        // $this->db->where('(final_rating = 0 OR final_rating = 1 OR final_rating = -1) AND assessed_reviewed = 1 AND validation = 1 AND attested = 1');
        ...
        $this->db->where('(b.final_rating = 0 OR b.final_rating = 1 OR b.final_rating = -1) AND b.assessed_reviewed = 1 AND b.validation = 1 AND b.attested = 1');
        $this->db->order_by('form_id', 'desc');
        $query = $this->db->get();
        $results = $query->result();
        foreach ($results as $key => $value) {
            $value->final_rating_date = date('M-d-Y',strtotime($value->validate_date));
    ...
    public function approve($postdata){
        ...
        $data = array(
            'final_rating' => $postdata['val'],
            'final_rating_date' => ($postdata['val'] == 1 ? date('Y-m-d H:i:s') : null),
