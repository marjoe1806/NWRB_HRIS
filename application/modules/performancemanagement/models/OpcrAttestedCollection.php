<?php
class OpcrAttestedCollection extends Helper {
    ...
    public function get_opcr(){
        ...
        // $this->db->where('(attested = 0 OR attested = 1 OR attested = -1) AND assessed_reviewed = 1 AND validation = 1');
        ...
        $this->db->where('(b.attested = 0 OR b.attested = 1 OR b.attested = -1) AND b.assessed_reviewed = 1 AND b.validation = 1');
        $this->db->order_by('form_id', 'desc');
        $query = $this->db->get();
        $results = $query->result();
    ...
    public function approve($postdata){
        ...
        $data = array(
            'attested' => $postdata['val'],
            'attested_date' => ($postdata['val'] == 1 ? date('Y-m-d H:i:s') : null),
            'attested_by' => $_SESSION['employee_id']
        );
