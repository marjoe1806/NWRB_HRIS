...
class DailyTimeRecordSummary extends MX_Controller {
	...
	function fetchRowsSummary(){
		...
		// $total_absence = 0;
		$self = 0;
		// if(in_array(17002,$_SESSION["sessionModules"])) $self = 1;
		$fetch_data = $this->DailyTimeRecordSummaryCollection->make_datatables_summary_dtr(@$_GET['Id'], @$_GET['EmployeeNumber'], $_GET['PayrollPeriod'], $_GET['PayrollPeriodId'], @$_GET['ShiftId'],$self);
		// var_dump(json_encode($fetch_data)); die();
		$attendance_settings = $this->DailyTimeRecordSummaryCollection->checkGlobalSettings();

		$formData['DTRlist'] = $fetch_data['records'];
		...
		foreach($fetch_data['records'] as $k => $v) {
			// Actual Logs
			$logs[$k]["actual_am_arrival"] 		= @$v['actual']['check_in']   == "" || @$v['actual']['check_in'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['check_in']));
			$logs[$k]["actual_am_departure"] 	= @$v['actual']['break_out']  == "" || @$v['actual']['break_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['break_out']));
			$logs[$k]["actual_pm_arrival"] 		= @$v['actual']['break_in']   == "" || @$v['actual']['break_in'] == "00:00:00" ? ""  : date('h:i a', strtotime($v['actual']['break_in']));
			// $logs[$k]["actual_pm_departure"] 	= @$v['actual']['check_out']  == "" || @$v['actual']['check_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['check_out']));
			$logs[$k]["actual_pm_departure"] 	= @$v['actual']['time_out']   == "" || @$v['actual']['time_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['time_out']));
			$logs[$k]["actual_overtime_in"] 	= @$v['actual']['ot_in']    == ""  || @$v['actual']['ot_in'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['ot_in']));
			$logs[$k]["actual_overtime_out"] 	= @$v['actual']['ot_out']    == "" || @$v['actual']['ot_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['actual']['ot_out']));
			$logs[$k]["actual_remarks"] = @$v['actual']['remarks'];

			// Adjustments
			$logs[$k]["adjustment_am_arrival"] 	 = @$v['adjustment']['check_in']  == "" || @$v['adjustment']['check_in'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['check_in']));
			$logs[$k]["adjustment_am_departure"] = @$v['adjustment']['break_out'] == "" || @$v['adjustment']['break_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['break_out']));
			$logs[$k]["adjustment_pm_arrival"] 	 = @$v['adjustment']['break_in']  == "" || @$v['adjustment']['break_in'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['break_in']));
			$logs[$k]["adjustment_pm_departure"] = @$v['adjustment']['time_out']  == "" || @$v['adjustment']['time_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['time_out']));
			$logs[$k]["adjustment_overtime_in"]  = @$v['adjustment']['ot_in']     == "" || @$v['adjustment']['ot_in'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['ot_in']));
			$logs[$k]["adjustment_overtime_out"] = @$v['adjustment']['ot_out']    == "" || @$v['adjustment']['ot_out'] == "00:00:00" ? "" : date('h:i a', strtotime($v['adjustment']['ot_out']));
			$logs[$k]["adjustment_remarks"] = @$v['adjustment']['remarks'];

			//Time Computation
			$logs[$k]["offset"]  = @$v['timeComputation']['offset'];
			$logs[$k]["ot_hrs"]  = @$v['timeComputation']['ot_hrs'];
			$logs[$k]["ot_mins"] = @$v['timeComputation']['ot_mins'];
			$logs[$k]["tardiness_hrs"]  = @$v['timeComputation']['tardiness_hrs'];
			$logs[$k]["tardiness_mins"] = @$v['timeComputation']['tardiness_mins'];
			$logs[$k]["ut_hrs"]  = @$v['timeComputation']['ut_hrs'];
			$logs[$k]["ut_mins"] = @$v['timeComputation']['ut_mins'];
			$logs[$k]["appr_offset_hrs"]  = @$v['timeComputation']['approved_offset_hrs'];
			$logs[$k]["appr_offset_mins"] = @$v['timeComputation']['approved_offset_mins'];



		}
			// var_dump(json_encode($logs)) ;die();
		...
		// var_dump(json_encode($formData)); die();
		$result['table'] = $this->load->view('helpers/dailytimerecordsummary.php', $formData, TRUE);
		$result['table2'] = $this->load->view('helpers/dailytimerecordsummary2.php', $formData, TRUE);
		$result['key'] = "viewDailyTimeRecordSummary";
		echo json_encode($result);
	...
	function getAttendanceV2($attendance,$ref, $key) {
		...
		/*-------------------------------------TIMEIN---------------------------------*/
			...
			if(isset($attendance['check_in']) && $attendance['check_in'] != null) {
				if(sizeof($attendance['time_in']) > 1) {
					// var_dump("here"); die();
					$time_in = $this->getEarliestTime($attendance['check_in']);
				} else {
