<?php
      ...
      $sql = " SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='tblemployees' AND TABLE_SCHEMA='".$this->db->database."' ";
		...
		function make_query($employment_status,$employee_id,$pay_basis,$payroll_period_id) {
			...
			if($self){
				...
				else $this->db->order_by($this->table.'.id', 'DESC');
			}	
		}
		...
		function make_datatables_summary_dtr($employee_id, $employee_number, $payroll_period, $payroll_period_id, $shift_id, $self){
			$result = $this->make_query_summary_dtr($employee_id, $employee_number, $payroll_period, $payroll_period_id, $shift_id, $self);
			return $result;
		}
		function make_query_summary_dtr($employee_id, $employee_number, $payroll_period, $payroll_period_id, $shift_id, $self) {
			if($self){
				$employee_id = $_SESSION["id"];
			}
			$dtr = array();
			$dtr['employee'] = $this->getEmployeeById($employee_id);
			$employee_number = $this->Helper->decrypt($dtr['employee'][0]['employee_number'], $employee_id);
			...
			for ($day = 1; $day <= 31; $day++) {
				...
				if($day >= $start[2] && $day <= $end[2]) {
					$dtr['records'][$current_day] = $this->getAttendanceByWorkingDaysDTR($current_day,$employee_number);
				}
				//  else {
	...
	function getAttendanceByWorkingDaysDTR($working_day,$employee_number){
		$logs = array(
			"actual" => array(
				"check_in" 	     => null,
				"break_out"      => null,
			...
			"timeComputation" => array(
				...
				"ut_hrs"         => null,
				"ut_mins"        => null,
				"approved_offset_hrs" => null,
				"approved_offset_mins" => null,
			),
		);
		$attendance = $this->getLogsDTR($working_day, $employee_number);
	...
	function getLogsDTR($working_day, $employee_number) {
		$employee_number = (int)$employee_number;

	
		$query = $this->db->select("*")->from("tbldtr")->where("transaction_date", $working_day)->where("CAST(scanning_no AS INT) = ", $employee_number)->get();
		$ret = $query->row();

		$haveLeave = $this->db->query("SELECT a.*,d.description
									...
									LEFT JOIN tblleavemanagementtype d ON d.code = a.type
									WHERE
										(
											(DATE(b.days_of_leave) = DATE('".$working_day."'))
											OR
											(	
												DATE(SUBSTRING_INDEX(b.days_of_leave,' - ',-1)) >= DATE('".$working_day."')
												AND
												DATE(SUBSTRING_INDEX(b.days_of_leave,' - ',1)) <= DATE('".$working_day."')
												AND
												a.type IN ('maternity','study','violence','rehab', 'benefits')
											)
										)
									AND CAST(DECRYPTER(c.employee_number,'sunev8clt1234567890',c.id) AS INT) = ".$employee_number." AND a.is_active = 1 AND a.status = 5 GROUP BY type")->result_array();
									
		$haveOB = $this->db->query("SELECT a.*
									FROM tbltimekeepinglocatorslips a
									LEFT JOIN tblemployees c ON a.employee_id = c.id
									WHERE
										(
											(
												(DATE(a.transaction_date) = DATE('".$working_day."'))
												OR
												(DATE(a.transaction_date_end) = DATE('".$working_day."'))
												OR
												(DATE('".$working_day."') BETWEEN DATE(a.transaction_date) AND DATE(a.transaction_date_end ))
											)
											AND
											(DAYOFWEEK(DATE('".$working_day."')) != 7
												AND DAYOFWEEK(DATE('".$working_day."')) != 1
											)
										)
									AND CAST(DECRYPTER(c.employee_number,'sunev8clt1234567890',c.id) AS INT) = ".$employee_number." 
									AND (a.status = 'APPROVED' OR a.status = 'COMPLETED')")->result_array();
									// (
									// 	(DATE(b.days_of_leave) = DATE('".$working_day."'))
									// 	OR
									// 	(DATE(SUBSTRING_INDEX(b.days_of_leave,' - ',-1)) = DATE('".$working_day."') OR DATE(SUBSTRING_INDEX(b.days_of_leave,' - ',1)) = DATE('".$working_day."'))
									// )
		// var_dump($working_day);
		// var_dump(json_encode($haveLeave));
		// var_dump(json_encode($haveOB));
		// var_dump(json_encode($ret));
		// var_dump($no_type);die();
		// die();
		if($ret || $haveLeave){
			// if($ret->check_in != NULL && $ret->break_out != NULL || $ret->break_in != NULL || $ret->check_out != NULL ||
			//    $ret->adjustment_check_in != NULL && $ret->adjustment_break_out != NULL || $ret->adjustment_break_in != NULL || $ret->adjustment_check_out != NULL) {
				return array(
						"actual" => array(
							"check_in" 	     => isset($ret->check_in) ? $ret->check_in : null,
							"break_out"      => isset($ret->break_out) ? $ret->break_out : null,
							"break_in" 	     => isset($ret->break_in) ? $ret->break_in : null,
							"time_out" 	     => isset($ret->check_out) ? $ret->check_out : null,
							"ot_in" 	     => isset($ret->ot_in) ? $ret->ot_in : null,
							"ot_out" 	     => isset($ret->ot_out) ? $ret->ot_out : null,
							"remarks"         => isset($ret->remarks) ? $ret->remarks : null,
						),
						"adjustment" => array(
							"check_in" 	     => isset($ret->adjustment_check_in) ? $ret->adjustment_check_in : null,
							"break_out"      => isset($ret->adjustment_break_out) ? $ret->adjustment_break_out : null,
							"break_in" 	     => isset($ret->adjustment_break_in) ? $ret->adjustment_break_in : null,
							"time_out" 	     => isset($ret->adjustment_check_out) ? $ret->adjustment_check_out : null,
							"ot_in" 	     => isset($ret->adjustment_ot_in) ? $ret->adjustment_ot_in : null,
							"ot_out" 	     => isset($ret->adjustment_ot_out) ? $ret->adjustment_ot_out : null,
							"remarks"         => (@$ret->adjustment_remarks != "Rest Day") ? ($haveLeave) ? $haveLeave[0]["description"] : @$ret->adjustment_remarks : @$ret->adjustment_remarks,
						),
						"timeComputation" => array(
							"offset" 	     => ($haveLeave) ? 0 : $ret->offset,
							"ot_hrs" 	     => ($haveLeave) ? 0 : $ret->ot_hrs,
							"ot_mins" 	     => ($haveLeave) ? 0 : $ret->ot_mins,
							"tardiness_hrs"  => ($haveLeave) ? 0 : $ret->tardiness_hrs,
							"tardiness_mins" => ($haveLeave) ? 0 : $ret->tardiness_mins,
							"ut_hrs"         => ($haveLeave) ? 0 : $ret->ut_hrs,
							"ut_mins"        => ($haveLeave) ? 0 : $ret->ut_mins,
							"approved_offset_hrs" => isset($ret->approve_offset_hrs) ? $ret->approve_offset_hrs : null,
							"approved_offset_mins" => isset($ret->approve_offset_mins) ? $ret->approve_offset_mins : null,
						),
					);
			// } else return array();
		}else if($ret || $haveOB){
			return array(
				...
				"adjustment" => array(
					...
					"ot_in" 	     => isset($ret->adjustment_ot_in) ? $ret->adjustment_ot_in : null,
					"ot_out" 	     => isset($ret->adjustment_ot_out) ? $ret->adjustment_ot_out : null,
					// "remarks"         => (@$ret->adjustment_remarks != "Rest Day") ? ($haveOB) ? 'OB - '.$haveOB[0]["control_no"] : @$ret->adjustment_remarks : @$ret->adjustment_remarks,
					"remarks"         => (@$ret->adjustment_remarks != "Rest Day") ? ($haveOB) ? $haveOB[0]["control_no"] : @$ret->adjustment_remarks : @$ret->adjustment_remarks,
				),
				"timeComputation" => array(
					"offset" 	     => ($haveOB) ? 0 : $ret->offset,
					"ot_hrs" 	     => ($haveOB) ? 0 : $ret->ot_hrs,
					"ot_mins" 	     => ($haveOB) ? 0 : $ret->ot_mins,
					"tardiness_hrs"  => ($haveOB) ? 0 : $ret->tardiness_hrs,
					"tardiness_mins" => ($haveOB) ? 0 : $ret->tardiness_mins,
					"ut_hrs"         => ($haveOB) ? 0 : $ret->ut_hrs,
					"ut_mins"        => ($haveOB) ? 0 : $ret->ut_mins,
					"approved_offset_hrs" => isset($ret->approve_offset_hrs) ? $ret->approve_offset_hrs : null,
					"approved_offset_mins" => isset($ret->approve_offset_mins) ? $ret->approve_offset_mins : null,
				),
			);
	...
	function getEmployeeShiftHistory($employee_number, $day) {
		...
		// 		shift_type,
		// 		shift_id,
		// 	   	DAY(STR_TO_DATE(previous_date_effectivity,'%m/%d/%Y')) AS shift_date_effectivity 
		// 	 FROM tblemployeesshifthistory
		// 	 WHERE employee_id = '".$employee_number."' 
		// 	 	   AND MONTH(STR_TO_DATE(previous_date_effectivity,'%m/%d/%Y')) = MONTH('".$day."')) ";
		$sql = "
				(SELECT 
					employee_id, 
					shift_type, 
			...
			UNION ALL
				(SELECT 
					id, 
					regular_shift, 
					CASE WHEN regular_shift = 1 THEN shift_id ELSE flex_shift_id END AS shift_id,
					DAY(STR_TO_DATE(shift_date_effectivity,'%m/%d/%Y')) 
				FROM tblemployees 
				WHERE id = '".$employee_number."' 
				AND MONTH(STR_TO_DATE(shift_date_effectivity,'%m/%d/%Y')) = MONTH('".$day."'))";
	...
	function getCurrentEmployeeShiftHistory($employee_number, $day) {
		// $schedule = array();
		$sql = "SELECT id AS employee_id, 
				regular_shift AS shift_type, 
				CASE WHEN regular_shift = 1 THEN shift_id ELSE flex_shift_id END AS shift_id,
				 1 AS shift_date_effectivity FROM tblemployees WHERE id = '".$employee_number."'";
		$query = $this->db->query($sql);
		// $schedule = $query->result_array();
		// var_export($this->db->last_query());die();
		return $query->result_array();
	...
	function checkAnte($time, $type){
		...
		public function checkOB($key){
		$helperDao = new HelperDao();
		$code = "1";
		$message = "Failed to import timelogs from excel.";

		$sql = 'SELECT remarks FROM tbltimekeepinglocatorslips WHERE transaction_date = "'.$key['date'].'" AND employee_id = "'.$key['employee_id'].'" AND (status ="APPROVED" OR status ="COMPLETED")';
		$query = $this->db->query($sql);
		if ($query->num_rows() == 0) { 
			...
			$message = "Data not Exist!";
			$data =  $query->result_array();
			$this->ModelResponse(0, 'Data not Found', $data);
