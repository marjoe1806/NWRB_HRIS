<?php
	...
	function checkAnte($time, $type){
		...
		$sql = 'SELECT remarks FROM tbltimekeepinglocatorslips WHERE transaction_date = "'.$key['date'].'" AND employee_id = "'.$key['employee_id'].'" AND status ="APPROVED"';
		$query = $this->db->query($sql);
		if ($query->num_rows() == 0) { 
			...
			$message = "Data not Exist!";
			$data =  $query->result_array();
			$this->ModelResponse(0, 'Data Found', $data);
