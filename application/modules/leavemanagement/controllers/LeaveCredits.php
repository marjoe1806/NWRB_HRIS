...
class LeaveCredits extends MX_Controller {
	
	public function __construct() {
		parent::__construct();
		$this->load->model('Helper');
		$this->load->model('LeaveCreditsCollection');
	...
	public function index() {
		...
		$page = "viewPayslip";
		$listData['key'] = $page;
		$viewData['table'] = $this->load->view("helpers/leavecreditslist",$listData,TRUE); 
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Leave Credits');
			Helper::setMenu('templates/menu_template');
			Helper::setView('leavecredits',$viewData,FALSE);
	...
	function fetchRows(){
		...
		$fetch_data = $this->LeaveCreditsCollection->make_datatables($division_id);
        $data = array();  
        foreach($fetch_data as $k => $row) {  
            ...
            $buttons .= ' <a class="viewServiceRecord" data-employee_id="'.$emp_id.'" style="text-decoration: none;" '
            		  . ' href="'. base_url().$this->uri->segment(1).'/'.$this->uri->segment(2).'/viewLeaveCredit" > '
            		  . ' <button class="btn btn-info btn-circle waves-effect waves-circle waves-float" data-toggle="tooltip" data-placement="top" title="View Balance of Leave Credit">'
        ...
        $output = array(  
            ...
            "recordsTotal"          =>     $this->LeaveCreditsCollection->get_all_data(),  
            "recordsFiltered"     	=>     $this->LeaveCreditsCollection->get_filtered_data($division_id),  
            "data"                  =>     $data  
        );  
	...
	public function formContainer(){
		...
		if (!$this->input->is_ajax_request()) {
		   show_404();
            ...
            $data["Data"] = $formData;
			$result['form'] = $this->load->view('forms/leavecreditsform.php', $data, TRUE);
		}
		echo json_encode($result);
	}

	public function viewLeaveCredit(){
		$formData = array();
		$result = array();
		if (!$this->input->is_ajax_request()) {
		   show_404();
			...
			$ret = $this->LeaveCreditsCollection->get_service_record($employee_id);
			$data['signatories'] = $this->LeaveCreditsCollection->getSignatories();
			$formData['key'] = $result['key'];
			...
			if(count($ret['employee']) > 0){
				...
				$formData['employee']['period'] = @$ledger[$year][sizeof($ledger[$year])-1]['period'];
				$formData['employee']['vl'] = @$ledger[$year][sizeof($ledger[$year])-1]['vl_balance'];
				$formData['employee']['sl'] = @$ledger[$year][sizeof($ledger[$year])-1]['sl_balance'];
				if(@$ledger[$year][sizeof($ledger[$year]) - 1]['period'] == ""){
					$formData['employee']['period'] = @$ledger[$year][sizeof($ledger[$year])-2]['period'];
					$formData['employee']['vl'] = @$ledger[$year][sizeof($ledger[$year])-2]['vl_balance'];
					$formData['employee']['sl'] = @$ledger[$year][sizeof($ledger[$year])-2]['sl_balance'];
				}

				// $forceLeave = $retleaveAvailable['availableforce'] > 0 ? $retleaveAvailable['availableforce'] : 0;
				// $force = 5 - $forceLeave;
				// $formData['employee']['vl'] = $formData['employee']['vl'] - $force;

				$leavebal = (double)@$formData['employee']['vl'] + (double)@$formData['employee']['sl'];

				// if($year >= 2020) {
				// 	$total_balance = $this->LeaveLedgerCollection->getLeaveBalance($employee_id);
				// 	if(sizeof($total_balance) > 0 && $total_balance[0]['total'] > $leavebal) {
				// 		$formData['employee']['vl'] = $total_balance[0]['vl'];
				// 		$formData['employee']['sl'] = $total_balance[0]['sl'];
				// 		$leavebal = $total_balance[0]['vl'] + (double)$total_balance[0]['sl'];
				// 	}
				// }
				$formData['employee']['leaveCreditsTotal'] = $leavebal;
