<?php
	class CTOLedgerCollection extends Helper {
		...
		{  
			...
			if($self){
				...
				if(isset($employee_id) && $employee_id != null) $this->db->where('tblemployees.id="'.$employee_id.'"');//die('hit');
				else{
					if((isset($division_id) && $division_id != null)) $this->db->where('tblemployees.division_id="'.$division_id.'"');//die('hit');
				}
		...
		{  
			...
			if($self){
				...
				if((isset($division_id) && $division_id != null))
					$this->db->where($this->table.'.division_id="'.$division_id.'"');//die('hit');
				else{
					$this->db->where('1=0');
				}
			}
		...
		function getEarned($month, $year, $employee_id) {
			$sql = "SELECT a.* FROM tbldtr a left join tblemployees b ON CAST(a.scanning_no AS INT) = CAST(DECRYPTER(b.employee_number,'sunev8clt1234567890',b.id) AS INT)
			WHERE YEAR(a.transaction_date) = '".$year."' AND MONTH(a.transaction_date) = '".$month."' AND b.id ='".$employee_id."' AND a.offset != 0 ORDER BY a.transaction_date ASC";
			$query = $this->db->query($sql);
			return $query->result_array();
		}

		function getUsed($month, $year, $employee_id) {
			$this->db->select("*");
			$this->db->from("tbloffsetting");
			$this->db->where('YEAR(offset_date_effectivity)',$year);
			$this->db->where('MONTH(offset_date_effectivity)',$month);
		...
		function getLeave($month,$employee_id,$year,$type){
			...
			// 			WHERE 
			// 				(SELECT MONTH(b.days_of_leave) FROM tblleavemanagementdaysleave b WHERE a.id = b.id LIMIT 0,1) = '".$month."'
			// 				AND
			// 				(SELECT YEAR(b.days_of_leave) FROM tblleavemanagementdaysleave b WHERE a.id = b.id LIMIT 0,1) = '".$year."'
			// 				AND employee_id = '".$employee_id."' AND type='".$type."' AND a.is_active = 1 AND status = 4 GROUP BY type";
			// $query = $this->db->query($sql);
			// return $query->result_array();
			// $terminal = $this->getTerminal($year,$employee_id);
			// $sql = "	SELECT a.*,SUM(a.number_of_days) AS number_of_days FROM tblleavemanagement a
			// WHERE	(
			// 			MONTH(a.inclusive_dates) = '".$month."'
			// 			OR
			// 			MONTH(SUBSTRING_INDEX(a.inclusive_dates,' - ',-1))  = '".$month."' 
			// 		) AND a.employee_id = '".$employee_id."' AND a.type='".$type."' AND a.is_active = 1 AND a.status = 4 ".(isset($terminal) ? "AND TIMESTAMP(a.is_terminal) < TIMESTAMP('" . $terminal["is_terminal"]."')": "" )." GROUP BY a.type";
			// var_dump($sql);die();
