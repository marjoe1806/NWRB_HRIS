...
class PagibigLCHL extends MX_Controller {
	...
	public function index() {
		...
		$listData = array();
		$viewData = array();
		$page = "viewPagibigLCHL";
		$listData['key'] = $page;
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Pagibig LCHL Remittance');
			Helper::setMenu('templates/menu_template');
			Helper::setView('pagibig_lchl',$viewData,FALSE);
	...
	public function viewPagibigLCHLSummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewPagibigLCHLSummary';
		if (!$this->input->is_ajax_request())
		   show_404();
		else {
			...
			foreach ($formData['payroll'] as $k => $v) {
				$formData['loanDeductions'][$v['employee_id']] = $ret->getAmortizedMPLLCHLLoans($v['employee_id'],$payroll_period_id,20);
                $formData['payrollinfo'][$v['employee_id']] = $ret->payrollinfo($v['employee_id']);
                if(sizeof($formData['loanDeductions'][$v['employee_id']]) <= 0){
                    unset($formData['payroll'][$k]);
                }
			}
			if(sizeof($formData['payroll']) > 0){
					$result['form'] = $this->load->view('helpers/pagibig_lchl.php', $formData, TRUE);
			}else{
