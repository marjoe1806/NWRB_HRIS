...
class PagibigMPL extends MX_Controller {
	...
	public function index() {
		...
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Pagibig MPL Remittance');
			Helper::setMenu('templates/menu_template');
			Helper::setView('pagibig_mpl',$viewData,FALSE);
	...
	public function viewPagibigMPLSummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewPagibigMPLSummary';
		if (!$this->input->is_ajax_request())
		   show_404();
		else {
			...
			foreach ($formData['payroll'] as $k => $v) {
				$formData['loanDeductions'][$v['employee_id']] = $ret->getAmortizedMPLLCHLLoans($v['employee_id'],$payroll_period_id,21);
			...
			if(sizeof($formData['payroll']) > 0){
					$result['form'] = $this->load->view('helpers/pagibig_mpl.php', $formData, TRUE);
			}else{
