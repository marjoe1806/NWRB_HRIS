...
class NWRBEA extends MX_Controller {
	...
	public function index() {
		...
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('NWRBEA Remittance');
			Helper::setMenu('templates/menu_template');
			Helper::setView('nwrbea',$viewData,FALSE);
	...
	public function viewNWRBEASummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewNWRBEASummary';
		if (!$this->input->is_ajax_request())
		   show_404();
		else {
			...
			if(sizeof($formData['payroll']) > 0){
					$result['form'] = $this->load->view('helpers/nwrbea.php', $formData, TRUE);
			}else{
