...
class PagibigContribution extends MX_Controller {
	...
	public function index() {
		...
		$listData = array();
		$viewData = array();
		$page = "viewPagibigContribution";
		$listData['key'] = $page;
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Pag-ibig Contribution');
			Helper::setMenu('templates/menu_template');
			Helper::setView('pagibigcontribution',$viewData,FALSE);
	...
	public function viewPagibigContributionSummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewPagibigContributionSummary';
		if (!$this->input->is_ajax_request())
		   show_404();
		else {
			...
			if(sizeof($formData['payroll']) > 0){
					$result['form'] = $this->load->view('helpers/pagibigcontribution.php', $formData, TRUE);
			}else{
