...
<?php if($key == 'viewPhilhealthContributionsAll'): ?>
			...
			<div class="table-responsive" style="width:100%; padding-bottom: 0">
				<table class="table" style="width:100%; font-size:10px !important; margin-bottom: 0">
					<thead style="background: #ddd">
						...
						<tr>
							<th class="text-center header-2 print-only" colspan="2">Summary of Monthly Philhealth Contributions</th>
						</tr>
						<tr>
				...
				<table id="datatables_details border" class="table table-hover table-striped border" style="width:100%; font-size:10px !important; margin-bottom: 0">
					...
					<tbody class="text-center">
						...
						<tr>
							...
							<td>
								<?php echo clean($row['philhealth']); ?>
							</td>
							<td>
								<?php echo number_format((float)abs($row['philhealth_amt']), 2, '.', ',') ; ?>
							</td>
							<td>
								<?php echo number_format((float)abs($row['philhealth_amt_employer']), 2, '.', ',') ; ?>
							</td>
							<td>
								<?php echo number_format((float)abs($row['philhealth_amt'] + $row['philhealth_amt_employer']), 2, '.', ',') ; ?>
							</td>
						</tr>
						<?php
							$totalemployeeshare += $row['philhealth_amt'];
							$totalemployershare += $row['philhealth_amt_employer'];
							$grandtotal += $row['philhealth_amt'] + $row['philhealth_amt_employer'];
							endif;
						} endif; ?>
