...
class OtherEarnings extends MX_Controller {
	...
	public function index() {
		Helper::rolehook(ModuleRels::OTHER_EARNINGS_SUMMARY_SUB_MENU);
		$listData = array();
		$viewData = array();
		...
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Other Earnings');
	...
	function fetchRows(){ 
        ...
        $fetch_data = $this->OtherEarningsCollection->make_datatables($pay_basis,$payroll_period_id);
        $data = array();  
        ...
        {  
            ...
            $buttons .= ' <a id="viewOtherEarningsForm" ' 
            		  . ' class="viewOtherEarningsForm" style="text-decoration: none;" '
            		  . ' href="'. base_url().$this->uri->segment(1).'/'.$this->uri->segment(2).'/viewOtherEarningsForm" '
            		  . $buttons_data
            		  . ' > '
            ...
            /*if($row->is_active == "1"){
	            $buttons .= ' <a ' 
	            		  . ' class="deactivateOtherEarnings btn btn-danger btn-circle waves-effect waves-circle waves-float" '
	        	...
	        	$buttons .= ' <a ' 
	        			  . ' class="activateOtherEarnings btn btn-success btn-circle waves-effect waves-circle waves-float" '
        ...
        $output = array(  
            ...
            "recordsTotal"          =>      $this->OtherEarningsCollection->get_all_data($pay_basis,$payroll_period_id),  
            "recordsFiltered"     	=>     $this->OtherEarningsCollection->get_filtered_data($pay_basis,$payroll_period_id),  
            "data"                  =>     $data  
        );  
        echo json_encode($output);  
    }
	public function viewOtherEarningsSummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewOtherEarningsSummary';
		if (!$this->input->is_ajax_request()) {
		   show_404();
		}
		else
		{
			...
			$formData["list"] = $this->OtherEarningsCollection->fetchOtherEarnings($pay_basis,$payroll_period_id);
			...
			$result['form'] = $this->load->view('helpers/employeeotherearnings.php', $formData, TRUE);
			/*} else {
