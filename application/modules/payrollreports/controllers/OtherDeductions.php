...
class OtherDeductions extends MX_Controller {
	...
	public function index() {
		Helper::rolehook(ModuleRels::OTHER_DEDUCTIONS_SUMMARY_SUB_MENU);
		$listData = array();
		$viewData = array();
	...
	function fetchRows(){ 
        ...
        $fetch_data = $this->OtherDeductionsCollection->make_datatables($pay_basis,$payroll_period_id);
        $data = array();  
        ...
        {  
            ...
            /*'<a  id="updateOtherDeductionsForm" 
            ...
            $buttons .= ' <a id="viewOtherDeductionsForm" ' 
            		  . ' class="viewOtherDeductionsForm" style="text-decoration: none;" '
            		  . ' href="'. base_url().$this->uri->segment(1).'/'.$this->uri->segment(2).'/viewOtherDeductionsForm" '
            		  . $buttons_data
            		  . ' > '
            ...
            /*if($row->is_active == "1"){
	            $buttons .= ' <a ' 
	            		  . ' class="deactivateOtherDeductions btn btn-danger btn-circle waves-effect waves-circle waves-float" '
	        	...
	        	$buttons .= ' <a ' 
	        			  . ' class="activateOtherDeductions btn btn-success btn-circle waves-effect waves-circle waves-float" '
        ...
        $output = array(  
            ...
            "recordsTotal"          =>      $this->OtherDeductionsCollection->get_all_data($pay_basis,$payroll_period_id),  
            "recordsFiltered"     	=>     $this->OtherDeductionsCollection->get_filtered_data($pay_basis,$payroll_period_id),  
            "data"                  =>     $data  
        );  
        echo json_encode($output);  
    }
	public function viewOtherDeductionsSummary(){
		$formData = array();
		$result = array();
		$result['key'] = 'viewOtherDeductionsSummary';
		if (!$this->input->is_ajax_request()) {
		   show_404();
		}
		else
		{
			...
			$formData["list"] = $this->OtherDeductionsCollection->fetchOtherDeductions($pay_basis,$payroll_period_id);
			...
			$result['form'] = $this->load->view('helpers/employeeotherdeductions.php', $formData, TRUE);
			/*} else {
