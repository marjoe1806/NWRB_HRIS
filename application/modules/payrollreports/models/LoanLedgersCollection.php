<?php
	class LoanLedgersCollection extends Helper {
		...
		var $table = "tbltransactionsloans";   
      	var $order_column = array('tblemployees.employee_number','tblemployees.first_name','tblfieldloans.code','tblfieldloanssub.code','amortization_per_month');
		...
		function make_query($pay_basis,$employee_id){
		    ...
		    $this->db->select(
		    	'tblemployees.*,
		    	(SELECT SUM(amount) FROM tbltransactionsloansamortization WHERE '.$this->table.'.id = tbltransactionsloansamortization.loan_entry_id AND tbltransactionsloansamortization.is_active = "1") AS amount,'.
		    	$this->table.'.*,
		    	tblfieldloans.code AS category_code,
		    	tblfieldloans.description AS category_description,
		    	tblfieldloanssub.code AS type_code,
		    	tblfieldloanssub.description AS type_description,
		    ...
		    $this->db->join("tblemployees","tbltransactionsloans.employee_id = tblemployees.id","left");
		    $this->db->join("tblfieldloans","tbltransactionsloans.loans_id = tblfieldloans.id","left");
		    $this->db->join("tblfieldloanssub","tbltransactionsloans.sub_loans_id = tblfieldloanssub.id","left");
		    $this->db->join("tblfieldperiodsettings","tbltransactionsloans.payroll_period_id = tblfieldperiodsettings.id","left");
		...
		function make_datatables($pay_basis,$employee_id){  
		    $this->make_query($pay_basis,$employee_id);  
		    if($_POST["length"] != -1)  
		    {  
		...
		function get_filtered_data($pay_basis,$employee_id){  
		     $this->make_query($pay_basis,$employee_id);
		     $query = $this->db->get();  
		     return $query->num_rows();  
		}       
		function get_all_data($pay_basis,$employee_id){  
		...
		function fetchLoanLedgers($pay_basis,$employee_id){
			$this->make_query($pay_basis,$employee_id);
		    $query = $this->db->get();
