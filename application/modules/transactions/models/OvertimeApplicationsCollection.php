<?php
	class OvertimeApplicationsCollection extends Helper {
		...
		var $table = "tbltransactionsovertimes";   
      	var $order_column = array("DECRYPTER(tblemployees.last_name,'sunev8clt1234567890',tblemployees.id)",
      							  "hrs_15","mins_15","hrs_125","mins_125","ot_percent","tax","is_active","username","");
		...
		{  
		    ...
		    if(isset($_GET['payroll_grouping_id']) && $_GET['payroll_grouping_id'] != null)
		    	$this->db->where($this->table.'.payroll_grouping_id="'.$_GET['payroll_grouping_id'].'"');
		...
		public function activeRows($params){
			...
			$code = "1";
			$message = "Overtime failed to activate.";
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully activated Overtime.";
		...
		public function inactiveRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Overtime failed to deactivate.";
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "0";
			$userlevel_sql = "	UPDATE ".$this->table." SET
									is_active = ? 
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully deactivated Overtime.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function addRows($params){
			...
			$message = "Overtime failed to insert.";
			$this->db->trans_begin();
			...
			if($this->db->trans_status() === TRUE){
				$code = "0";
				$message = "Successfully inserted Overtime.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
		...
		public function updateRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Overtime failed to update.";
			...
			if($this->db->trans_status() === TRUE){
				$code = "0";
				$message = "Successfully updated Overtime.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				$this->db->trans_commit();
				return true;	
