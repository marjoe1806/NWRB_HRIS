<?php
	class LeaveGroupingCollection extends Helper {
		...
		public function hasRowsActive(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldleavegrouping where is_active = '1'";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function hasRowsById($params){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldleavegrouping where id = ?";
		...
		public function activeRows($params){
			...
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tblfieldleavegrouping SET
									is_active = ? 
								WHERE id = ? ";
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully activated leave grouping.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function inactiveRows($params){
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully deactivated leave grouping.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function addRows($params){
			...
			$this->db->insert('tblfieldleavegrouping', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
				$message = "Successfully inserted leave grouping.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function updateRows($params){
			...
			if ($this->db->update('tblfieldleavegrouping',$params) === FALSE){
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
			}	
			else {
				$code = "0";
				$message = "Successfully updated leave grouping.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;	
			}
