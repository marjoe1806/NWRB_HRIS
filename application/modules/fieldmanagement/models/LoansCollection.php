<?php
	class LoansCollection extends Helper {
		...
		var $table = "tblfieldloans";
      	var $order_column = array('code','description','comments','is_active');
		...
		public function hasRowsSub($loan_id){
			$helperDao = new HelperDao();
			$data = array();
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldloanssub WHERE loan_id = ?";
		...
		public function hasRowsActive(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldloans where is_active = '1'";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function hasRowsSubActive(){
			...
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldloanssub where is_active = '1' AND loan_id = ?";
			$query = $this->db->query($sql,array($loan_id));
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function activeRows($params){
			...
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tblfieldloans SET
									is_active = ? 
								WHERE id = ? ";
		...
		public function activeSubRows($params){
			...
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tblfieldloanssub SET
									is_active = ? 
								WHERE id = ? ";
			$userlevel_params = array($data['Status'],$data['Id']);
			$this->db->query($userlevel_sql,$userlevel_params);
		...
		public function addRows($params){
			...
			$this->db->insert('tblfieldloans', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
		...
		public function addSubRows($params){
			...
			$params['modified_by'] = Helper::get('userid');
			$message = "Loans failed to insert.";
			$this->db->insert('tblfieldloanssub', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
				$message = "Successfully inserted loan.";
		...
		public function updateRows($params){
			...
			if ($this->db->update('tblfieldloans',$params) === FALSE){
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
			}	
			else {
