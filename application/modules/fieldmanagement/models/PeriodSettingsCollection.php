<?php
	class PeriodSettingsCollection extends Helper {
		...
		function make_query($status,$month){
		...
		function make_datatables($status,$month){
		    $this->make_query($status,$month);
			if($_GET["length"] != -1) $this->db->limit($_GET['length'], $_GET['start']);
			
		    $query = $this->db->get();
		    return $query->result();
		}
		function get_filtered_data($status,$month){
			$this->make_query($status,$month);
			$query = $this->db->get();
		    return $query->num_rows();
		}
		function get_all_data($status,$month){
		    $this->db->select($this->table."*");
			$this->db->from($this->table);
			if($status != "ALL") $this->db->where($this->table.".is_active",$status);
			if($month != "ALL") $this->db->where("MONTH(".$this->table.".start_date)",$month);
		...
		public function hasRowsWeeklyPeriodSettings(){
			$helperDao = new HelperDao();
			$data = array();
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldperiodsettingsweekly WHERE payroll_period_id = ".$_POST['id']." ORDER BY start_date ASC";
			$query = $this->db->query($sql);
		...
		public function hasRowsMonthly(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldperiodsettings WHERE is_semi_monthly = 0 ORDER BY payroll_period DESC";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function hasRowsById(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldperiodsettings";
		...
		public function hasRowsActive($pay_basis = ""){
			if($pay_basis != ""){
				if($pay_basis == "Permanent" || $pay_basis == "Contractual" || $pay_basis == "Temporary" || $pay_basis == "Contract of Service"){
