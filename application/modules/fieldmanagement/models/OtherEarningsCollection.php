<?php
	class OtherEarningsCollection extends Helper {
		...
		var $table = "tblfieldotherearnings";
      	var $order_column = array('','earning_code','description','is_active');
		...
		public function hasRowsActive(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldotherearnings where is_active = '1'";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function activeRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Other earnings level failed to activate.";
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tblfieldotherearnings SET
									is_active = ? 
								WHERE id = ? ";
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully activated other earnings.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function inactiveRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Other earnings level failed to deactivate.";
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "0";
			$userlevel_sql = "	UPDATE tblfieldotherearnings SET
									is_active = ? 
			...
			if($this->db->affected_rows() > 0){
				$code = "0";
				$message = "Successfully deactivated other earnings.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function addRows($params){
			...
			$message = "Other earnings failed to insert.";
			$this->db->insert('tblfieldotherearnings', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
				$message = "Successfully inserted other earnings.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;		
			}	
		...
		public function updateRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Other earnings failed to update.";
			$params['is_taxable'] = isset($params['is_taxable'])? "1" : "0" ;
			$this->db->where('id', $params['id']);
			if ($this->db->update('tblfieldotherearnings',$params) === FALSE){
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
			}	
			else {
				$code = "0";
				$message = "Successfully updated other earnings.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;	
			}
