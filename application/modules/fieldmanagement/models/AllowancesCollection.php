<?php
	class AllowancesCollection extends Helper {
		...
		public function hasRows(){
			...
			$sql = " SELECT * FROM tblfieldallowances";
		...
		public function hasRowsActive(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tblfieldallowances where is_active = '1'";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function activeRows($params){
			$helperDao = new HelperDao();
			$code = "1";
			$message = "Allowance failed to activate.";
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tblfieldallowances SET
									is_active = ? 
								WHERE id = ? ";
		...
		public function addRows($params){
			...
			$this->db->insert('tblfieldallowances', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
		...
		public function updateRows($params){
			...
			if ($this->db->update('tblfieldallowances',$params) === FALSE){
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
			}	
			else {
				$code = "0";
				$message = "Successfully updated allowance.";
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
				return true;	
			}
