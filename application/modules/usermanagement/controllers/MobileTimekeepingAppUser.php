...
class MobileTimekeepingAppUser extends MX_Controller {
	
	public function __construct() {
		parent::__construct();
		$this->load->model('Helper');
		$this->load->model('MobileTimekeepingAppUserCollection');
	...
	public function index() {
		Helper::rolehook(ModuleRels::USER_MOBILE_TIMEKEEPING_APP_MENU);
		$listData = array();
		$viewData = array();
		...
		$listData['locations'] = $this->MobileTimekeepingAppUserCollection->getActiveLocations();
		$listData['departments'] = $this->MobileTimekeepingAppUserCollection->getActiveDepartments();

		$viewData['table'] = $this->load->view("helpers/mobileuserconfiglist",$listData,TRUE); 
		if (!$this->input->is_ajax_request()) {
			Helper::setTitle('Mobile Timekeeping App User');
			Helper::setMenu('templates/menu_template');
			Helper::setView('mobiletimekeepingappuser',$viewData,FALSE);
	...
	public function addMobileUserConfigForm($year=false, $months=false){
		$formData = array();
		$result = array();
		...
		if (!$this->input->is_ajax_request()) {
		   show_404();
			...
			$formData['locations'] = $this->MobileTimekeepingAppUserCollection->getActiveLocations();

			$formData['key'] = $result['key'];
			$result['form'] = $this->load->view('forms/mobiletimekeepingappuserform.php', $formData, TRUE);
		}
		echo json_encode($result);
	}
