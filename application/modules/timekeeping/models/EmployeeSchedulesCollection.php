<?php
	class EmployeeSchedulesCollection extends Helper {
		...
		public function hasRowsRegular(){
			...
			$sql = " SELECT * FROM tbltimekeepingemployeeschedules";
		...
		public function hasRowsShifting(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tbltimekeepingemployeeschedules WHERE shift_type = 1";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function hasRowsActive(){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tbltimekeepingemployeeschedules where is_active = '1' ";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function hasRowsWeeklyEmployeeSchedules($id){
			...
			$code = "1";
			$message = "No data available.";
			$sql = " SELECT * FROM tbltimekeepingemployeeschedulesweekly WHERE shift_code_id = " . $id . " ";
			$query = $this->db->query($sql);
			$userlevel_rows = $query->result_array();
			$data['details'] = $userlevel_rows;
			if(sizeof($userlevel_rows) > 0){
		...
		public function addRows($params){
			$helperDao = new HelperDao();
			$isDescriotionExist = $this->db->select("*")->from("tbltimekeepingemployeeschedules")
			...
			$this->db->insert('tbltimekeepingemployeeschedules', $params);
			
			// var_dump($isDescriotionExist); die();
		...
		public function updateRows($params){
			$helperDao = new HelperDao();
			$isDescriotionExist = $this->db->select("*")->from("tbltimekeepingemployeeschedules")->where("id !=",$params['id'])->where("description",$params['description'])->get()->num_rows();
			$code = "1";
			$message = $isDescriotionExist < 1 ? "Employee Schedule failed to update." : "Shift description already exist.";
				
			if($isDescriotionExist < 1){
				$this->db->where('id', $params['id']);
				if ($this->db->update('tbltimekeepingemployeeschedules',$params) === FALSE){
					$helperDao->auditTrails(Helper::get('userid'),$message);
					$this->ModelResponse($code, $message);
				}	
				else {
		...
		public function activeRows($params){
			...
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tbltimekeepingemployeeschedules SET
									is_active = ? 
								WHERE id = ? ";
		...
		public function activeRowsWeekly($params){
			...
			$data['Id'] 	= isset($params['id'])?$params['id']:'';
			$data['Status'] 		= "1";
			$userlevel_sql = "	UPDATE tbltimekeepingemployeeschedulesweekly SET
									is_active = ? 
								WHERE id = ? ";
			$userlevel_params = array($data['Status'],$data['Id']);
			$this->db->query($userlevel_sql,$userlevel_params);
		...
		public function addRowsWeekly($params){
			...
			$this->db->insert('tbltimekeepingemployeeschedulesweekly', $params);
			if($this->db->affected_rows() > 0)
			{
				$code = "0";
		...
		public function updateRowsWeekly($params){
			...
			$params['is_restday'] = isset($params['is_restday']) ? "1" : "0" ;
			$this->db->where('id', $params['id']);
			if ($this->db->update('tbltimekeepingemployeeschedulesweekly',$params) === FALSE){
				$helperDao->auditTrails(Helper::get('userid'),$message);
				$this->ModelResponse($code, $message);
			}	
			else {
