$(function () {
					...
					yes: {
						...
						action: function () {
							...
							$.ajax({
								...
								url:
									commons.baseurl +
									"timekeepingreports/ActualAttendance/checkPost",
								data: employee_id,
								dataType: "json",
								success: function (result) {
									$("#defaultModalLabel").html("Actual Attendance");
									...
									result.forEach((value) => {
										...
										$.ajax({
											...
											url:
												commons.baseurl +
												"timekeepingreports/ActualAttendance/fetchRowsSummary" +
												plus_url,
											dataType: "json",
											success: function (result) {
												$(
	...
	$(document).on("click", "#summarizeAllEmployeeDailyTimeRecord", function (e) {
		...
		if (pay_basis == "") {
			...
			getFields.employeeModal(pay_basis, "", division_id, specific);
		}
	});
	...
	//Ajax non-forms
	$(document).on("click", ".viewActualAttendanceForm", function (e) {
		e.preventDefault();
		my = $(this);
		id = my.attr("data-id");
		url = my.attr("href");
			...
			success: function (result) {
				...
				table = $("#datatables").DataTable({
					...
					ajax: {
						url:
							commons.baseurl +
							"timekeepingreports/ActualAttendance/fetchRows" +
							plus_url,
						type: "POST",
					},
					columnDefs: [
				...
				button =
					'<a id="viewActualAttendanceAll" href="' +
					commons.baseurl +
					'timekeepingreports/ActualAttendance/viewActualAttendanceAll">' +
					'<button type="button" class="btn btn-block btn-lg btn-success waves-effect">' +
					'<i class="material-icons">people</i> ActualAttendance Summary' +
					"</button>" +
					"</a>";
	...
	function PrintElem(elem) {
		...
		mywindow.document.write(
			"<style> * { font-family: arial; text-align: center; font-size: 12px; color: #000; background: #fff } body { -webkit-print-color-adjust: exact !important; size: auto; } @page { margin: 3mm 5mm 3mm 5mm; } .fixed-height { height: 20pt !important } .text-danger { color: #c00808 !important } .text-danger td { color: #c00808 !important } small { font-size: 10px } .text-left { text-align: left } .text-right { text-align: right } table { width: 50% } .border table { border-collapse: collapse } .border td, .border th { border: 1px solid #000; } @media print { .no-print, .no-print * { display: none; } } </style>"
		);
