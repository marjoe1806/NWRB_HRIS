$(document).ready(function() {
    ...
    function ipcr_reviews(params) {
        $('#ipcr_reviews').DataTable({
            ...
            ajax: {
                url: baseURL + "performancemanagement/IpcrReviews/get_ipcr",
                type: "POST",
                data: params,
            },
            columns: [{
                ...
                {
                    "data": function(data) {
                        return `<button class="btn btn-info btn-circle waves-effect waves-circle waves-float pull-right view" 
                            ...
                            <button class="btn btn-success btn-circle waves-effect waves-circle waves-float pull-right viewprint" 
                                    data-id = "${data.form_id}"
                                    data-toggle="tooltip" 
                                    data-placement="top" 
                                    title="" 
    ...
    function step3() {
        ...
        $('#approved_date').show();
        $('.form_table').find('input[type=checkbox]').show();
        $('.form_table td').find('input[type=text]').show();
        $('.form_table td').find('input[type=number]').show();
        ...
        $.each(data.data, function(k, v) {
            ...
            if (v.weight_of_output == "Final-Rating") {
                var rate = parseInt(v.q1) + parseInt(v.e2) + parseInt(v.t3) + parseInt(v.a4);
                console.log(rate);
                var rateAvg = rate / 4;
            ...
            if (v.weight_of_output == "Final-Rating") {
                ...
                console.log(rate);
                var rateAvg = rate / 4;
                if (Math.round(rateAvg) == 1) {
                    rateOtp = "P";

                } else if (Math.round(rateAvg) == 2) {
                    rateOtp = "U";

                } else if (Math.round(rateAvg) == 3) {
                    rateOtp = "S";

                } else if (Math.round(rateAvg) == 4) {
                    rateOtp = "VS";

                } else if (Math.round(rateAvg) == 5) {
                    rateOtp = "O";
                } else {
                    rateOtp = "NA"
                }
        ...
        $('#final_avg_ratings').text('Final Average Ratings');
    ...
    $(document).on("click", "#getIpcrReview", function(e) {
        e.preventDefault();
        my = $(this);
        url = my.attr("href");
        ...
        ipcr_reviews({ division_id, employee_id })
    });
    ...
    $(document).on("click", ".view", function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        var url = baseURL + 'performancemanagement/IpcrReviews/view_ipcr';
    ...
    $(document).on("click", ".viewprint", function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        var url = baseURL + 'performancemanagement/IpcrReviews/print_ipcr';
        $.post(url, { id: id }).done(function(result) {
            result = JSON.parse(result);
            data = result;
            form = 'update_form';
    ...
    $(document).on("click", ".update", function(e) {
        var url = baseURL + 'performancemanagement/IpcrReviews/update';
