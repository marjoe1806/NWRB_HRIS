$(document).ready(function(){
    ...
    var count_support = 1;
    $('#dpcr_form').validate();
    $(".dpcr_date").inputmask("yyyy-mm-dd", {placeholder: "yyyy-mm-dd",});
    $.when(
        getFields.division(),
    ...
    $(document).on("click", ".remove", function (e) {
        e.preventDefault();
        $(this).blur();
        if($(`#dpcr_table_second tbody .${$(this).attr("data-key")}`).length == 2 ){
            $(this).closest('tr').find('input').val('');
        }
        else if(!$(this).closest('tr').next('tr').hasClass($(this).attr("data-key")) && $(`#dpcr_table_second tbody .${$(this).attr("data-key")}`).length != 2){
            console.log("here+")
    ...
    $(document).on("submit", "#dpcr_form", function (e) {
        e.preventDefault();
        
        var url = baseURL + 'performancemanagement/Dpcr/add_form';
        var data = $('#dpcr_form').serializeArray();
