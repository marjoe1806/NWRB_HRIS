$(function(){
    ...
    var table;
    var tbl_url = commons.baseurl+ "transactions/OtherEarningEntries/fetchRows"
    // loadTable();
    $.when(
        getFields.month(),
        // getFields.location()
    ...
    $(document).on('click','#printOtherEarningEntries',function(e){
        e.preventDefault();
    ...
    //Confirms
    $(document).on('click','.activateOtherEarningEntries,.deactivateOtherEarningEntries',function(e){
        e.preventDefault();
        me = $(this);
        url = me.attr('href');
        var id = me.attr('data-id');
        content = 'Are you sure you want to proceed?';
        if(me.hasClass('activateOtherEarningEntries')){
            content = 'Are you sure you want to activate selected Other Earning?';
        }
        else if(me.hasClass('deactivateSubOtherEarningEntries')){
            content = 'Are you sure you want to deactivate selected Other Earning?';
        }
        data = {id: id};
        $.confirm({
                                    ...
                                    success: function(result){
                                        if(result.Code == "0"){
                                            if(result.hasOwnProperty("key")){
                                                switch(result.key){
                                                    case 'activateOtherEarningEntries':
                                                    case 'deactivateOtherEarningEntries':
    ...
    $(document).on('click','#searchEmployeeEarning',function(e){
        e.preventDefault();
        my = $(this)
        url = my.attr('href');
    ...
    //Ajax non-forms
    $(document).on('click','#addOtherEarningEntriesForm,.updateOtherEarningEntriesForm,.viewOtherEarningEntriesForm,.viewOtherEarningEntriesUpdates',function(e){
        e.preventDefault();
        my = $(this)
        id = my.attr('data-id');
        url = my.attr('href');  
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'addOtherEarningEntries':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Add New Other Earning');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'viewOtherEarningEntries':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Other Earning Details');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'updateOtherEarningEntries':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Update Other Earning');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'viewOtherEarningEntriesUpdates':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-lg');
                            $('#myModal .modal-dialog').addClass('full-screen');
                            $('#myModal .modal-title').html('Other Earning History');
                    ...
                    $.when(
                        ...
                        if(result.key =="viewOtherEarningEntries"){
                        ...
                        if(result.key == "addOtherEarningEntries"){
                            $('.search_entry #pay_basis').val(pay_basis).change();
                            $('.search_entry #division_id').val(division_id).change();
    ...
    //Ajax Forms
    $(document).on('submit','#addOtherEarningEntries,#updateOtherEarningEntries',function(e){
        e.preventDefault();
        var form = $(this)
        content = "Are you sure you want to proceed?";
        division_id = $("#myModal #division_id").val();
        earning_id = $("#myModal #earning_id").val();
        ...
        if(division_id == "" || division_id == "undefined"){
            ...
            return false;
        } else if (earning_id == "" || earning_id == "undefined") {
            $.alert({
                title: '<label class="text-danger">Failed</label>',
                content: "Please select Type of Earning.",
            });
            return false;
        }
        if(form.attr('id') == "addOtherEarningEntries"){
            content = "Are you sure you want to add Other Earning?";
        }
        if(form.attr('id') == "updateOtherEarningEntries"){
    ...
    function loadUpdatesTable(id){
        ...
        table = $('#datatables-updates').DataTable({  
            ...
            "ajax":{  
                url:commons.baseurl+ "transactions/OtherEarningEntriesUpdates/fetchRows"+plus_url,   
                type:"POST",
            },  
            oLanguage: {sProcessing: '<div class="preloader pl-size-sm">'
                                    +'<div class="spinner-layer pl-red-grey">'
