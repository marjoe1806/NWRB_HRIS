$(function () {
  ...
  $(document).on("click", ".addParticipant", function (e) {
    ...
    // $("select[id^=employee_select]:last").val($("select[id^=employee_select]").find("option:eq(0)").val())
  ...
  $(document).on(
    "click",
    "#addForeignRecordForm,.updateForeignRecordForm,.previewForeignRecord",
    function (e) {
      e.preventDefault();
      me = $(this);
      id = me.attr("data-id");
      ...
      $.ajax({
        ...
        success: function (result) {
          ...
          if (result.hasOwnProperty("key")) {
            switch (result.key) {
              case "addForeignRecord":
                page = "";
                $("#myModal .modal-dialog").attr(
                  "class",
                  "modal-dialog modal-md"
                );
                $("#myModal .modal-title").html("Add Foreign Record");
                ...
                break;
              case "previewForeignRecord":
                page = "";
                $("#myModal .modal-dialog").attr(
                  "class",
                  "modal-dialog modal-lg"
                );
                $("#myModal .modal-title").html("Preview Foreign Record");
                ...
                var getParticipants = (function () {
                  ...
                  url2 =
                    commons.baseurl +
                    "trainingmonitoring/ForeignRecord/getParticipants";
                  data2 = { id: id };
                  $.ajax({
                    async: false,
                    url: url2,
                ...
                $("#myModal").modal("show");
                break;
              case "updateForeignRecord":
                $("#myModal .modal-dialog").attr(
                  "class",
                  "modal-dialog modal-md"
                );
                $("#myModal .modal-title").html("Update ForeignRecord");
                $("#myModal .modal-body").html(result.form);
                $.each(me.data(), function (i2, v2) {
                  $("." + i2)
                    .val(me.data(i2))
  ...
  //Ajax Forms
  $(document).on("submit", "#addForeignRecord,#updateForeignRecord", function (
    e
  ) {
    e.preventDefault();
    var form = $(this);
    content = "Are you sure you want to proceed?";
    if (form.attr("id") == "addForeignRecord") {
      content = "Are you sure you want to add foreign record?";
    }
    if (form.attr("id") == "updateForeignRecord") {
      content = "Are you sure you want to update foreign record?";
    }
