$(function(){
    ...
    $(document).on('click','#addBalanceRow',function(e){
        ...
        //Month Population
        month_options = $('.row_0').find('#leave_month').html()
        $('.row_'+index).find('.month_select').html('')
        month_select = '<select data-container="body" class="leave_month form-control" name="leave_month['+index+']" id="leave_month" data-live-search="true" required>'                              
        month_select += month_options  
        month_select += '</select>'
        $('.row_'+index).find('.month_select').html(month_select)
            ...
            $.ajax({
                ...
                success: function(result){
                    ...
                    table = $('#datatables').DataTable({  
                           ...
                           "order":[],  
                           "ajax":{  
                                url:commons.baseurl+ "leavemanagement/ExhaustedPosting/fetchRows"+plus_url,  
                                type:"POST"  
                           },  
                           "columnDefs":[  
                                {  
    ...
    //Confirms
    $(document).on('click','.activateExhaustedPosting,.deactivateExhaustedPosting',function(e){
        e.preventDefault();
        me = $(this);
        url = me.attr('href');
        var id = me.attr('data-id');
        content = 'Are you sure you want to proceed?';
        if(me.hasClass('activateExhaustedPosting')){
            content = 'Are you sure you want to activate selected Balance?';
        }
        else if(me.hasClass('deactivateExhaustedPosting')){
                                    ...
                                    success: function(result){
                                        if(result.Code == "0"){
                                            if(result.hasOwnProperty("key")){
                                                switch(result.key){
                                                    case 'activateExhaustedPosting':
                                                    case 'deactivateExhaustedPosting':
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.hasOwnProperty("key")){
                    ...
                    if(result.key != "employeeExhaustedPosting"){
                        $.when(
                            // getFields.location(),
                            // getFields.division()
                            getFields.leaveGrouping()
