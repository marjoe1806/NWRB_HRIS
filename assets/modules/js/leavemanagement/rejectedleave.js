$(function(){
    ...
    $(document).on('change','#leave_kind_search',function(e){
        url = commons.baseurl + "leavemanagement/RejectedLeave/";
        url += "?Kind="+$(this).val();
        window.location = url;
    })
    ...
    //Ajax non-forms
    $(document).on('click','.viewRejectedLeaveRegularDetails,.viewRejectedLeaveSpecialDetails',function(e){
        e.preventDefault();
        me = $(this)
        id = me.attr('data-id');
        url = me.attr('href');  
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'viewRejectedLeaveSpecialDetails':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-lg');
                            $('#myModal .modal-title').html('Rejected Leave Details');
                            ...
                            var getLeaveDates = function() {
                                var temp = null;
                                url2 = commons.baseurl + "leavemanagement/RejectedLeave/getLeaveDates";
                                data2 = {id:id}
                                $.ajax({
                                    async: false,
                                    url: url2,
                            ...
                            break;
                        case 'viewRejectedLeaveRegularDetails':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-lg');
                            $('#myModal .modal-title').html('Rejected Leave Details');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
