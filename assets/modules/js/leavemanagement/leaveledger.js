$(function () {
  ...
  $(document).on("click", ".viewLeaveLedgerForm", function (e) {
    e.preventDefault();
    $.alert({
      title: '<label class="text-info">Message</label>',
      content: "Please open previous year’s Leave Card if you don’t see an updated starting balance.",
    });
  ...
  $(document).on("click", "#viewLeaveLedgerSummary", function (e) {
    ...
    if (month == "") {
      ...
      $.ajax({
        type: "POST",
        url: commons.baseurl + "leavemanagement/LeaveLedger/getEmployeeList",
        data: {
          pay_basis: pay_basis,
          division_id: division_id,
          specific: specific,
          ...
          action: function () {
            ...
            if (total_data > 0) {
              ...
              $.ajax({
                ...
                url:
                  commons.baseurl +
                  "leavemanagement/LeaveLedger/leaveledgerContainer",
                dataType: "json",
                success: function (result) {
                  if (result.hasOwnProperty("key")) {
                    switch (result.key) {
  ...
  function getEmployeeLeaveLedger(
    location_id,
    division_id,
    employee_id,
    month,
    ...
    url2 =
      commons.baseurl + "leavemanagement/LeaveLedger/viewLeaveLedgerSummary/";
    $.ajax({
      type: "POST",
      url: url2,
      data: {
      ...
      success: function (result) {
        ...
        table = $("#datatables").DataTable({
          ...
          ajax: {
            url:
              commons.baseurl +
              "leavemanagement/LeaveLedger/fetchRows" +
              plus_url,
            type: "POST",
          },
          columnDefs: [
        ...
        button =
          '<a id="viewLeaveLedgerSummary" href="' +
          commons.baseurl +
          'leavemanagement/LeaveLedger/viewLeaveLedgerSummary">' +
  ...
  function loadTable() {
    ...
    table = $("#datatables").DataTable({
      ...
      ajax: {
        url:
          commons.baseurl + "leavemanagement/LeaveLedger/fetchRows" + plus_url,
        type: "POST",
      },
      columnDefs: [
        {
