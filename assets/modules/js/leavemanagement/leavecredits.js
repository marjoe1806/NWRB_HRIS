$(function () {
  ...
  $(document).on("click", ".viewServiceRecord", function (e) {
    ...
    $.ajax({
      type: "POST",
      url: url,
      data: { employee_id: employee_id, key: "viewLeaveCredit" },
      dataType: "json",
      success: function (result) {
        page = my.attr("id");
        if (result.hasOwnProperty("key")) {
          switch (result.key) {
            case "viewLeaveCredit":
              page = "";
  ...
  $(document).on("click", "#viewLeaveCreditsSummary", function (e) {
    e.preventDefault();
    division_id = $(".search_entry #division_id").val();
    if (division_id == "") {
      $.alert({
  ...
  $(document).on("click", "#submitUserCheckList", function (e) {
    ...
    $.confirm({
      ...
      content: "Are you sure you want to generate leave credits?",
      type: "orange",
      buttons: {
        confirm: {
          btnClass: "btn-blue",
          action: function () {
            ...
            if (total_data > 0) {
              $.ajax({
                ...
                url:
                  commons.baseurl +
                  "leavemanagement/LeaveCredits/formContainer",
                dataType: "json",
                success: function (result) {
                  if (result.hasOwnProperty("key")) {
                    if (result.key === "formContainer") {
                      ...
                      $("#module")
                        ...
                        .each(function (k, v) {
                          ...
                          queue = getEmployeeLeaveCredit(employee_id, k + 1);
                          if (queue) {
                            percentage =
  ...
  function getEmployeeLeaveCredit(employee_id, queue) {
    var url = commons.baseurl + "leavemanagement/LeaveCredits/viewLeaveCredit/";
    $.ajax({
      type: "POST",
      url: url,
      async: false,
      data: {
        ...
        key: "viewLeaveCreditSummary",
        queue: queue,
      },
      dataType: "json",
      success: function (result) {
        if (result.hasOwnProperty("key")) {
          if (result.key === "viewLeaveCreditSummary") {
    ...
    //   success: function (result) {
        ...
        table = $("#datatables").DataTable({
          ...
          ajax: {
            url:
              commons.baseurl +
              "leavemanagement/LeaveCredits/fetchRows?DivisionId=" +
              $("#division_id").val(),
            type: "POST",
          },
          columnDefs: [
