$(function () {
	...
	$(document).on(
		"click",
		"#addInterviewSchedulesForm, .updateInterviewSchedulesForm, .viewInterviewSchedulesForm",
		function (e) {
			e.preventDefault();
			my = $(this);
			data = my.data();
						...
						if (result.hasOwnProperty("key")) {
							...
							switch (status_key) {
								case 'addInterviewSchedule':
									...
									$('#myModal').modal('show');
									break;
								case 'updateInterviewSchedule':
									$('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
									$('#myModal .modal-title').html('Update Interview Schedule');
									$('#myModal .modal-body').html(result.form);
									$('#myModal').modal('show');
	...
	// ajax forms
	$(document).on('submit', '#addInterviewSchedule, #updateInterviewSchedule', function(e) {
		e.preventDefault();
		var form = $(this);
		...
		if (form.attr('id') == "addInterviewSchedule") {
			content = "Are you sure you want to add interview schedule?";
		}
		if (form.attr('id') == 'updateInterviewSchedule') {
			content = "Are you sure you want to update interview schedule? This will notify the applicant of the new schedule";
		}
	...
	table = $("#datatables").DataTable({
		...
		initComplete: function () {
			var input = $('.dataTables_filter input').unbind(),
				self = this.api(),
				$searchButton = $(
					'<button id="search-interview-schedule" class="btn bg-purple btn-circle waves-effect waves-circle waves-float">'
				)
					.html('<i class="material-icons">search</i>')
					.click(function () {
						if (!$("#search-interview-schedule").is(":disabled")) {
							$("#search-interview-schedule").attr("disabled", true);
				...
				if	($("#search-interview-schedule").length === 0) {
		...
		drawCallback: function (settings) {
			...
			$('#search-interview-schedule').removeAttr('disabled');
			$('#datatables button').removeAttr('disabled');
		},
		ajax: {
			url: commons.baseurl + "recruitment/InterviewSchedules/fetchRows" + plus_url,
			type: 'POST',
		},
		oLanguage: {
			sProcessing:
