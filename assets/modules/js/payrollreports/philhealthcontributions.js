$(function () {
	...
	$(document).on('click', '#summarizeAllPhilhealthContributions', function (e) {
		e.preventDefault();
		my = $(this)
		url = my.attr('href');
		...
		if (pay_basis == "") {
			...
			$.ajax({
				type: "POST",
				url: commons.baseurl + "payrollreports/PhilhealthContributions/fetchRowsSummaryAll" + plus_url,
				dataType: "json",
				success: function (result) {
					if (result.hasOwnProperty("key")) {
						switch (result.key) {
							case 'viewPhilhealthContributionsAll':
								if(result.success == true) {
									$('#myModal .modal-dialog').attr('class', 'modal-dialog modal-lg');
									$('#myModal .modal-dialog').attr('style', 'width: 95%');
									modal_title = "Monthly Philhealth Contributions Summary"
