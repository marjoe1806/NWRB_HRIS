$(function () {
	...
	$(document).on('change', '#payroll_period_id', function () {
		...
		$('#myModal #payroll_period_modal').html(payroll_period2);
	});

	$(document).on('click', '#summarizeAllTaxContributions', function (e) {
		e.preventDefault();
		my = $(this)
		url = my.attr('href');
		...
		if (pay_basis == "") {
			...
			$.ajax({
				type: "POST",
				url: commons.baseurl + "payrollreports/TaxContributions/fetchRowsSummaryAll" + plus_url,
				dataType: "json",
				success: function (result) {
					if (result.hasOwnProperty("key")) {
						switch (result.key) {
							case 'viewTaxContributionsAll':
								if (result.success == true) {
									$('#myModal .modal-dialog').attr('class', 'modal-dialog modal-lg');
									$('#myModal .modal-dialog').attr('style', 'width: 95%');
									modal_title = "Monthly Tax Contributions Summary"
