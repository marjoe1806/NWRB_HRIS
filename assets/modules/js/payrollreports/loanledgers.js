$(function(){
    ...
    $(document).on('change','#pay_basis ',function(e){
        pay_basis = $(this).val();
        $.when(
            getFields.employeeByPayBasis(pay_basis)
        ).done(function(){
            $.AdminBSB.select.activate();  
        })
    })
    ...
    //Ajax non-forms
    $(document).on('click','#viewLoanLedgersSummary',function(e){
        e.preventDefault();
        my = $(this)
        url = my.attr('href');
        ...
        $.ajax({
            type: "POST",
            url: url,
            data: {employee_id:employee_id,pay_basis:pay_basis},
            dataType: "json",
            success: function(result){
                page = my.attr('id');
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'viewLoanLedgersSummary':
                            page="";
    ...
    $(document).on('click','#computePayroll',function(e){
        ...
        plus_url = '?PayBasis='+pay_basis+'&Id='+employee_id
        $.ajax({
            type: "POST",
            url: url,
            dataType: "json",
            success: function(result){
                ...
                table = $('#datatables').DataTable({  
                       ...
                       "order":[],  
                       "ajax":{  
                            url:commons.baseurl+ "payrollreports/LoanLedgers/fetchRows"+plus_url,  
                            type:"POST"  
                       },  
                       "columnDefs":[  
                            {  
                ...
                button =    '<a id="viewLoanLedgersSummary" href="'+commons.baseurl+'payrollreports/LoanLedgers/viewLoanLedgersSummary">'
                            +    '<button type="button" class="btn btn-block btn-lg btn-success waves-effect">'
                            +        '<i class="material-icons">description</i> Loan  Ledger Summary'
                            +    '</button>'
                            +'</a>'
