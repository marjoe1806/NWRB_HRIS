$(function(){
    ...
    $(document).on('click','#printDepartments',function(e){
        e.preventDefault();
        PrintElem("servicerecords-container");
    })
    //Confirms
    $(document).on('click','.activateDepartments,.deactivateDepartments',function(e){
        e.preventDefault();
        me = $(this);
        url = me.attr('href');
        var id = me.attr('data-id');
        content = 'Are you sure you want to proceed?';
        if(me.hasClass('activateDepartments')){
            content = 'Are you sure you want to activate selected departments?';
        }
        else if(me.hasClass('deactivateSubDepartments')){
            content = 'Are you sure you want to deactivate selected sub departments?';
        }
        data = {id: id};
        $.confirm({
                                    ...
                                    success: function(result){
                                        if(result.Code == "0"){
                                            if(result.hasOwnProperty("key")){
                                                switch(result.key){
                                                    case 'activateDepartments':
                                                    case 'deactivateDepartments':
    ...
    //Ajax non-forms
    $(document).on('click','#addDepartmentsForm,.updateDepartmentsForm',function(e){
        e.preventDefault();
        me = $(this)
        id = me.attr('data-id');
        url = me.attr('href');  
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'addDepartments':
                            ...
                            $('#myModal .modal-title').html('Add New Department');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'generateDepartments':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                        ...
                        case 'updateDepartments':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Update Department');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
    ...
    //Ajax Forms
    $(document).on('submit','#addDepartments,#updateDepartments',function(e){
        e.preventDefault();
        var form = $(this)
        content = "Are you sure you want to proceed?";
        if(form.attr('id') == "addDepartments"){
            content = "Are you sure you want to add departments?";
        }
        if(form.attr('id') == "updateDepartments"){
            content = "Are you sure you want to update departments?";
        }
    ...
    function loadTable(){
        ...
        table = $('#datatables').DataTable({  
            ...
            "ajax":{  
                url:commons.baseurl+ "fieldmanagement/Departments/fetchRows?status="+$("#status").val(),
                type:"GET",
            },  
