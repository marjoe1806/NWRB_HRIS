$(function(){
    ...
    $(document).on('click','#printOtherDeductions',function(e){
        e.preventDefault();
        PrintElem("servicerecords-container");
    })
    //Confirms
    $(document).on('click','.activateOtherDeductions,.deactivateOtherDeductions',function(e){
        e.preventDefault();
        me = $(this);
        url = me.attr('href');
        var id = me.attr('data-id');
        content = 'Are you sure you want to proceed?';
        if(me.hasClass('activateOtherDeductions')){
            content = 'Are you sure you want to activate selected other deduction?';
        }
        else if(me.hasClass('deactivateSubOtherDeductions')){
                                    ...
                                    success: function(result){
                                        if(result.Code == "0"){
                                            if(result.hasOwnProperty("key")){
                                                switch(result.key){
                                                    case 'activateOtherDeductions':
                                                    case 'deactivateOtherDeductions':
    ...
    //Ajax non-forms
    $(document).on('click','#addOtherDeductionsForm,.updateOtherDeductionsForm',function(e){
        e.preventDefault();
        me = $(this)
        id = me.attr('data-id');
        url = me.attr('href');  
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'addOtherDeductions':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Add New Other Deduction Record');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'generateOtherDeductions':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Other Deduction Record');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'updateOtherDeductions':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Update Other Deduction Record');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            $.each(me.data(),function(i,v){
                                $('.'+i).val(me.data(i)).change();
                            });
                            if (me.data("is_taxable") == '1')
    ...
    //Ajax Forms
    $(document).on('submit','#addOtherDeductions,#updateOtherDeductions',function(e){
        e.preventDefault();
        var form = $(this)
        content = "Are you sure you want to proceed?";
        if(form.attr('id') == "addOtherDeductions"){
            content = "Are you sure you want to add other deduction?";
        }
        if(form.attr('id') == "updateOtherDeductions"){
    ...
    function loadTable(){
        ...
        table = $('#datatables').DataTable({  
            ...
            "ajax":{  
                url:commons.baseurl+ "fieldmanagement/OtherDeductions/fetchRows?status="+$("#status").val(),
                type:"GET",
            },  
