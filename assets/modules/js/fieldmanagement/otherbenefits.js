$(function () {
	...
	$(document).on('click', '#printOtherBenefits', function (e) {
		e.preventDefault();
		PrintElem("servicerecords-container");
	})
	//Confirms
	$(document).on('click', '.activateOtherBenefits,.deactivateOtherBenefits', function (e) {
		e.preventDefault();
		me = $(this);
		url = me.attr('href');
		var id = me.attr('data-id');
		content = 'Are you sure you want to proceed?';
		if (me.hasClass('activateOtherBenefits')) {
			content = 'Are you sure you want to activate selected other benefit?';
		} else if (me.hasClass('deactivateSubOtherBenefits')) {
			content = 'Are you sure you want to deactivate selected sub other benefit?';
		}
		data = {
			id: id
		};
									...
									success: function (result) {
										if (result.Code == "0") {
											if (result.hasOwnProperty("key")) {
												switch (result.key) {
													case 'activateOtherBenefits':
													case 'deactivateOtherBenefits':
	...
	//Ajax non-forms
	$(document).on('click', '#addOtherBenefitsForm,.updateOtherBenefitsForm', function (e) {
		e.preventDefault();
		me = $(this)
		id = me.attr('data-id');
		url = me.attr('href');
		$.ajax({
			...
			success: function (result) {
				...
				if (result.hasOwnProperty("key")) {
					switch (result.key) {
						case 'addOtherBenefits':
							page = "";
							$('#myModal .modal-dialog').attr('class', 'modal-dialog modal-md');
							$('#myModal .modal-title').html('Add New Other Benefit Record');
							$('#myModal .modal-body').html(result.form);
							$('#myModal').modal('show');
							break;
						case 'generateOtherBenefits':
							page = "";
							$('#myModal .modal-dialog').attr('class', 'modal-dialog modal-md');
							$('#myModal .modal-title').html('Other Benefit Record');
							$('#myModal .modal-body').html(result.form);
							$('#myModal').modal('show');
							break;
						case 'updateOtherBenefits':
							$('#myModal .modal-dialog').attr('class', 'modal-dialog modal-md');
							$('#myModal .modal-title').html('Update Other Benefit Record');
							$('#myModal .modal-body').html(result.form);
							$('#myModal').modal('show');
	...
	//Ajax Forms
	$(document).on('submit', '#addOtherBenefits,#updateOtherBenefits', function (e) {
		e.preventDefault();
		var form = $(this)
		content = "Are you sure you want to proceed?";
		if (form.attr('id') == "addOtherBenefits") {
			content = "Are you sure you want to add other benefit?";
		}
		if (form.attr('id') == "updateOtherBenefits") {
    ...
    function loadTable(){
        ...
        table = $('#datatables').DataTable({  
            ...
            "ajax":{  
                url:commons.baseurl+ "fieldmanagement/OtherBenefits/fetchRows?status="+$("#status").val(),
                type:"GET",
            },  
