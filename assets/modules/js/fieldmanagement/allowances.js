$(function(){
    ...
    $(document).on('click','#printAllowances',function(e){
        e.preventDefault();
        PrintElem("servicerecords-container");
    })
    //Confirms
    $(document).on('click','.activateAllowances,.deactivateAllowances',function(e){
        e.preventDefault();
        me = $(this);
        url = me.attr('href');
        var id = me.attr('data-id');
        content = 'Are you sure you want to proceed?';
        if(me.hasClass('activateAllowances')){
            content = 'Are you sure you want to activate selected allowance?';
        }
        else if(me.hasClass('deactivateSubAllowances')){
            content = 'Are you sure you want to deactivate selected sub allowance?';
        }
        data = {id: id};
        $.confirm({
                                    ...
                                    success: function(result){
                                        if(result.Code == "0"){
                                            if(result.hasOwnProperty("key")){
                                                switch(result.key){
                                                    case 'activateAllowances':
                                                    case 'deactivateAllowances':
    ...
    //Ajax non-forms
    $(document).on('click','#addAllowancesForm,.updateAllowancesForm',function(e){
        e.preventDefault();
        me = $(this)
        id = me.attr('data-id');
        url = me.attr('href');  
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'addAllowances':
                            ...
                            $('#myModal .modal-title').html('Add New Allowance');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'generateAllowances':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Employee Allowance');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'updateAllowances':
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-md');
                            $('#myModal .modal-title').html('Update Allowance');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
    ...
    //Ajax Forms
    $(document).on('submit','#addAllowances,#updateAllowances',function(e){
        e.preventDefault();
        var form = $(this)
        content = "Are you sure you want to proceed?";
        if(form.attr('id') == "addAllowances"){
            content = "Are you sure you want to add allowance?";
        }
        if(form.attr('id') == "updateAllowances"){
