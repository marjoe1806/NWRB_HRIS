$(function() {
  ...
  $(document).on("show.bs.modal", "#myModal", function() {
    ...
    $("#time_suspension").bootstrapMaterialDatePicker({
      date: false,
      format: "hh:mm A"
    });
  ...
  $(document).on("click", "#printHolidays", function(e) {
    e.preventDefault();
    PrintElem("servicerecords-container");
  });
  //Confirms
  $(document).on("click", ".activateHolidays,.deactivateHolidays", function(e) {
    e.preventDefault();
    me = $(this);
    url = me.attr("href");
    var id = me.attr("data-id");
    content = "Are you sure you want to proceed?";
    if (me.hasClass("activateHolidays")) {
      content = "Are you sure you want to activate selected holiday?";
    } else if (me.hasClass("deactivateSubHolidays")) {
      content = "Are you sure you want to deactivate selected sub holiday?";
    }
    data = { id: id };
    $.confirm({
                  ...
                  success: function(result) {
                    if (result.Code == "0") {
                      if (result.hasOwnProperty("key")) {
                        switch (result.key) {
                          case "activateHolidays":
                          case "deactivateHolidays":
  ...
  //Ajax non-forms
  $(document).on("click", "#addHolidaysForm,.updateHolidaysForm", function(e) {
    e.preventDefault();
    me = $(this);
    id = me.attr("data-id");
    url = me.attr("href");
        ...
        if (result.hasOwnProperty("key")) {
          switch (result.key) {
            case "addHolidays":
              ...
              $("#myModal .modal-title").html("Add New Holiday");
            ...
            case "generateHolidays":
              ...
              $("#myModal .modal-title").html("Employee Holiday");
              $("#myModal .modal-body").html(result.form);
              $("#myModal").modal("show");
              break;
            case "updateHolidays":
              ...
              $("#myModal .modal-title").html("Update Holiday");
              $("#myModal .modal-body").html(result.form);
              $("#myModal").modal("show");
  ...
  $(document).on("submit", "#addHolidays,#updateHolidays", function(e) {
    ...
    var form = $(this);
    content = "Are you sure you want to proceed?";
    if (form.attr("id") == "addHolidays") {
      content = "Are you sure you want to add holiday?";
    }
    if (form.attr("id") == "updateHolidays") {
      content = "Are you sure you want to update holiday?";
    }
  ...
  function loadTable(){
      ...
      table = $('#datatables').DataTable({  
          ...
          "ajax":{  
              url:commons.baseurl+ "fieldmanagement/Holidays/fetchRows?status="+$("#status").val()+"&month="+$("#month").val(),
              type:"GET",
          },  
