$(function () {
				...
				buttons: {
					yes: {
						...
						action: function () {
							...
							$.ajax({
								type: "POST",
								url: commons.baseurl + "payrollreports/DailyTimeRecordSummary/checkPost",
								data: employee_id,
								dataType: "json",
								success: function (result) {
									...
									result.forEach(value => {
										...
										$.ajax({
											type: "GET",
											url: commons.baseurl + "payrollreports/DailyTimeRecordSummary/fetchRowsSummary" + plus_url,
											dataType: "json",
											success: function (result) {
	...
	function loadTable() {
		...
		table = $('#datatables').DataTable({
			...
			"order": [],
			"ajax": {
				url: commons.baseurl + "payrollreports/DailyTimeRecordSummary/fetchRows" + plus_url,
				type: "POST"
			},
			"columnDefs": [{
				"targets": [0],
	...
	function PrintElem(elem) {
		...
		mywindow.document.write('<style> * { font-family: arial; text-align: center; font-size: 12px; color: #000; background: #fff } body { -webkit-print-color-adjust: exact !important; size: auto; } @page { margin: 3mm 5mm 3mm 5mm; } .fixed-height { height: 16pt !important } .text-danger { color: #c00808 !important } .text-danger td { color: #c00808 !important } small { font-size: 10px } .text-left { text-align: left } .text-right { text-align: right } table { width: 50% } .border table { border-collapse: collapse } .border td, .border th { border: 1px solid #000; } @media print { .no-print, .no-print * { display: none; } } </style>')
