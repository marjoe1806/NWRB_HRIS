$(function(){
    ...
    //Ajax non-forms
    $(document).on('click','#addLoansForm,.updateLoansForm,.viewSubLoansForm,.viewBatch,.viewQuestioner,.updateScore',function(e){
        e.preventDefault();
        me = $(this)
        id = me.attr('data-id');
        url = me.attr('href');  
                ...
                if(result.hasOwnProperty("key")){
                    switch(result.key){
                        case 'updateScore':
                            ...
                            $('#myModal .modal-title').html('Update Score');
                        ...
                        case 'viewQuestioner':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-lg');
                            $('#myModal .modal-title').html('View Questioner');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'viewBatch':
                            page="";
                            $('#myModal .modal-dialog').attr('class','modal-dialog modal-lg');
                            $('#myModal .modal-title').html('View Batch');
                            $('#myModal .modal-body').html(result.form);
                            $('#myModal').modal('show');
                            break;
                        case 'addLoans':
    ...
    //Ajax Forms
    $(document).on('submit','#addLoans,#updateLoans,#addSubLoans,#updateScore',function(e){
        e.preventDefault();
        var form = $(this)
        content = "Are you sure you want to proceed?";
        ...
        if(form.attr('id') == "updateScore"){
            content = "Are you sure you want to update score?";
        }
                                    ...
                                    success: function(result){
                                        if(result.hasOwnProperty("key")){
                                            if(result.code == "0"){
                                                ...
                                                self.setContent('Score updated successfully');
                                                self.setTitle('<label class="text-success">success</label>');
                                                $('#myModal .modal-body').html('');
                                                $('#myModal').modal('hide');
                                                loadTable();
    ...
    function loadTable(){
        ...
        table = $('#datatables').DataTable({  
            ...
            "ajax":{  
                url:commons.baseurl+ "competencymanagement/CompetencyTestExamResult/fetchRows?status="+$("#status").val(),
                type:"GET",
            },  
